...
class Demo:
    ...
    def vision_check_object(self, cmd: dict) -> bool:
        """
        Phase 3C:
        Render top camera, detect colored objects, and verify that
        the requested language-grounded object is visible.
        """
        try:
            ...
            grounded = perception_ground_object(cmd, detections)

            print("[VISION] Detections:", detections)
            print("[VISION] Grounded:", grounded)

            if not grounded.get("visible", True):
                obj = grounded.get("obj", "unknown object")
                with self._console_lock:
                    self.console_status = f"❌ Vision check failed: {obj} not visible"
                print(f"[VISION] Object not visible: {obj}")
                return False

            return True

        except Exception as e:
            print("[VISION] Warning: vision check failed internally:", e)
            return True
